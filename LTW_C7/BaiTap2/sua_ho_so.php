...
if ($id <= count($employees)) {
    $employee = $employees[$id - 1];
}
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && $_POST['id'] <= count($employees)) {
    $id = (int)$_POST['id'];
    $employees[$id - 1] = [
        "id" => $id,
        ...
        "ngay_tao" => $employee['ngay_tao'] // Keep original creation date
    ];
    ...
    <div class="container">
        <header>
            <div style="display: flex; align-items: center;">
                <div class="logo">
                    <img src="BAITAP2/logo_baclieu.png" alt="Logo Đại học Bạc Liêu">
                </div>
                <div class="title">
        ...
        <main>
            ...
            <form action="sua_ho_so.php" method="post">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <p><label>Họ tên:</label><input type="text" name="ho_ten" value="<?php echo $employee ? $employee['ho_ten'] : ''; ?>" required></p>
                <p><label>Chức vụ:</label><input type="text" name="chuc_vu" value="<?php echo $employee ? $employee['chuc_vu'] : ''; ?>" required></p>
                <p><label>Đơn vị:</label><input type="text" name="don_vi" value="<?php echo $employee ? $employee['don_vi'] : ''; ?>" required></p>
                <p><label>Ngày sinh:</label><input type="date" name="ngay_sinh" value="<?php echo $employee ? $employee['ngay_sinh'] : ''; ?>" required></p>
                <p><label>Địa chỉ:</label><input type="text" name="dia_chi" value="<?php echo $employee ? $employee['dia_chi'] : ''; ?>" required></p>
                <p><input type="submit" value="Lưu"></p>
            </form>
            <?php if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])): ?>
                <div class="success">Đã cập nhật hồ sơ của nhân viên ID: <?php echo $id; ?></div>
            <?php endif; ?>
        </main>
    </div>
