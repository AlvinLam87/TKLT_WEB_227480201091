...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_term'])) {
    ...
    if (!empty($search_term)) {
        foreach ($employees as $employee) {
            if (stripos($employee['name'], $search_term) !== false || 
                stripos((string)$employee['id'], $search_term) !== false ||
                stripos($employee['position'], $search_term) !== false ||
                stripos($employee['department'], $search_term) !== false) {
...
<head>
    ...
    <title>Tìm hồ sơ nhân viên</title>
    ...
    <style>
        * {
        ...
        .success:hover, .no-results:hover {
    ...
    <div class="container">
        ...
        <nav class="sidebar">
            ...
            <ul>
                ...
                <li><a href="tim_ho_so.php" class="active"><i class="fas fa-search"></i>Tìm hồ sơ</a></li>
            </ul>
        ...
        <main>
            <h2>Tìm hồ sơ nhân viên</h2>
            <form action="tim_ho_so.php" method="post">
                <p>
                    <label>Từ khóa tìm kiếm:</label>
                    <input type="text" name="search_term" value="<?php echo htmlspecialchars($search_term); ?>" placeholder="Nhập tên, ID, chức vụ hoặc phòng ban" required>
                </p>
            ...
            <?php if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($search_term)): ?>
                ...
                <?php else: ?>
                    <div class="no-results">
                        Không tìm thấy hồ sơ nào phù hợp với từ khóa "<?php echo htmlspecialchars($search_term); ?>".
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </main>
