...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && $_POST['id'] >= 1 && $_POST['id'] <= count($employees)) {
    $id = (int)$_POST['id'] - 1;
...
<body>
    <div class="container">
        ...
        <main>
            ...
            <form action="xoa_ho_so.php" method="post">
                <p><label>ID nhân viên:</label><input type="number" name="id" min="1" max="<?php echo count($employees); ?>" required></p>
                <p><input type="submit" value="Xóa"></p>
            </form>
            <?php if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])): ?>
                <div class="success">Đã xóa hồ sơ của nhân viên ID: <?php echo $_POST['id']; ?></div>
            <?php endif; ?>
        </main>
    </div>
